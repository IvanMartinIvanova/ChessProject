...
Plataforma::Plataforma(const Vector3D& lim1, const Vector3D& lim2, const Color& col) :
	...
	poscabj{(limite1.x + limite2.x) / 2,limite2.y,(limite1.z + limite2.z) / 2},
	poscarr{ (limite1.x + limite2.x) / 2,limite1.y,(limite1.z + limite2.z) / 2 }
{
}
...
{
	limite1.x = poscarr.x;
	limite1.y = poscarr.y + 0.3*sin(counter);
	limite1.x = poscarr.x;

	limite2.x = poscabj.x;
	limite2.y = poscabj.y + 0.3*sin(counter);
	limite2.x = poscabj.x;
